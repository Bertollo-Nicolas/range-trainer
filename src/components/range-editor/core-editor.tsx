...
import { RangeEditorData, Action, MixedColor, HandAction, DEFAULT_ACTION, FOLD_ACTION } from '@/types/range-editor'
...
export const CoreEditor = forwardRef<CoreEditorRef, CoreEditorProps>(function CoreEditor({ selectedRange, pendingRangeSelection, onSave, onUnsavedChanges, onRangeUpdated, onRangeSelectionConfirmed, onRangeSelectionCancelled, className }, ref) {
  ...
  const [originalData, setOriginalData] = useState<RangeEditorData | null>(null)
  ...
  useEffect(() => {
    if (selectedRange && title !== (selectedRange.data?.editorData?.title || selectedRange.name)) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await TreeService.update(selectedRange.id, updatePayload)
