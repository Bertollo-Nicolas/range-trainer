-- Migration pour les nouvelles tables Anki avec support FSRS
...
-- RLS pour les cartes v2
ALTER TABLE anki_cards_v2 ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own cards v2" ON anki_cards_v2
  FOR SELECT USING (
    deck_id IN (
      SELECT id FROM anki_decks 
      WHERE user_id = auth.uid()
    )
  );

CREATE POLICY "Users can insert their own cards v2" ON anki_cards_v2
...
CREATE POLICY "Users can update their own cards v2" ON anki_cards_v2
  FOR UPDATE USING (
    deck_id IN (
      SELECT id FROM anki_decks 
      WHERE user_id = auth.uid()
...
CREATE POLICY "Users can delete their own cards v2" ON anki_cards_v2
  FOR DELETE USING (
    deck_id IN (
      SELECT id FROM anki_decks 
      WHERE user_id = auth.uid()
...
-- RLS pour les révisions v2
ALTER TABLE anki_reviews_v2 ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own reviews v2" ON anki_reviews_v2
  FOR SELECT USING (
    card_id IN (
      SELECT id FROM anki_cards_v2 c
      JOIN anki_decks d ON c.deck_id = d.id
      WHERE d.user_id = auth.uid()
    )
  );

CREATE POLICY "Users can insert their own reviews v2" ON anki_reviews_v2
  FOR INSERT WITH CHECK (
    card_id IN (
      SELECT id FROM anki_cards_v2 c
      JOIN anki_decks d ON c.deck_id = d.id
...
-- RLS pour les sessions v2
ALTER TABLE anki_study_sessions_v2 ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own sessions v2" ON anki_study_sessions_v2
  FOR SELECT USING (
    deck_id IS NULL OR deck_id IN (
      SELECT id FROM anki_decks 
      WHERE user_id = auth.uid()
    )
  );

CREATE POLICY "Users can insert their own sessions v2" ON anki_study_sessions_v2
  FOR INSERT WITH CHECK (
    deck_id IS NULL OR deck_id IN (
      SELECT id FROM anki_decks 
      WHERE user_id = auth.uid()
...
CREATE POLICY "Users can update their own sessions v2" ON anki_study_sessions_v2
  FOR UPDATE USING (
    deck_id IS NULL OR deck_id IN (
      SELECT id FROM anki_decks 
      WHERE user_id = auth.uid()
...
-- RLS pour les paramètres de deck v2
ALTER TABLE anki_deck_settings_v2 ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own deck settings v2" ON anki_deck_settings_v2
  FOR SELECT USING (
    deck_id IN (
      SELECT id FROM anki_decks 
      WHERE user_id = auth.uid()
...
CREATE POLICY "Users can upsert their own deck settings v2" ON anki_deck_settings_v2
  FOR ALL USING (
    deck_id IN (
      SELECT id FROM anki_decks 
      WHERE user_id = auth.uid()
